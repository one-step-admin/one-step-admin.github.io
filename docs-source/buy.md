...
3. 支付成功后需提供：
    ...
    - **Gitlab 用户名**。专业版源码托管在 [Gitlab](https://gitlab.com/) 私有仓库，需提供你的 Gitlab 用户名，我们会邀请你加入仓库。
4. 进入专业版[私有仓库](https://gitlab.com/hooray0905/one-step-admin-pro)，获取源码。

> 问：Gitlab 无法注册怎么办？
> 
> 答：Gitlab 注册时会用到谷歌 reCAPTCHA 验证，请确保你的当前网络环境可以访问外网；或者你可以选择使用 Github 账号进行登录，绕过 Gitlab 注册机制。除此之外，你也可以联系作者帮忙注册。

> 问：为什么使用 Gitlab ，而不使用 Github ？
> 
> 答：虽然 Gitlab 注册稍微麻烦一点，但注册成功后，后续进行登录或其它操作时，对网络环境则没有要求，并且大多时候 Gitlab 的访问速度会比 Github 更快。
...
本框架在产品生命周期内提供长期维护，专业版用户可加入专业版微信群（群内提供框架技术支持），除此之外，每一位专业版用户可**免费绑定**一位开发者（不支持换绑），绑定的开发者同样可加入专业版微信群，额外绑定更多开发者为 200 元/人。
